// ExifRemoverShellExt.cpp : Implementation of CExifRemoverShellExt

#include "stdafx.h"
#include "ExifRemoverShellExt.h"
...
CExifRemoverShellExt::CExifRemoverShellExt()
{
	m_hApplicationBmp = LoadBitmap( _AtlBaseModule.GetModuleInstance(),
                           MAKEINTRESOURCE(IDB_BITMAP) );	
}


STDMETHODIMP CExifRemoverShellExt::Initialize ( 
...
HRESULT hr = S_OK;
	...
	std::wstring filetype_exe=L".EXE";
        ...
        lRet = reg.Open ( HKEY_CURRENT_USER,
                          _T("Software\\4dots Software\\Exif Remover"),KEY_QUERY_VALUE);
		...
		if (lRet!=ERROR_SUCCESS)
		{
			...
			wcscpy (szFp,L".png|||.jpg|||.jpeg|||.gif|||.bmp|||.tiff|||");
			szFpL=wcslwr(szFp);
		}
		else
		{
		...
		lRet=reg.QueryStringValue(_T("File Extensions"),szFp,&dwSizeFp);
		...
		L".JPG",L".JPEG",L".GIF",L".BMP",L".PNG",L".TIFF"};

		//L".MP3",L".FLAC",L".WMA",L".AAC",L".M4A",L".OGG",L".AIF",L".AIFF",L".WAV",L".AC3",L"WV",L".APE",L".MPC",L".OPUS",L".MP2",L".AU",L".AMR",L".AVI",L".MP4",L".MPEG",L".MPG",L".MOV",L".MKV",L".FLV",L".WMV",L".3GP",L".VOB",L".SWF",L".3G2",L".ASF",L".M2P",L".M2TS",L".M2V",L".M4V",L".MP2",L".OGM",L".OGV",L".QT",L".RM",L".RMVB",L".TS"};
		
		//L".AVI", L".MP4", L".MPEG", L".MPG", L".MOV", L".MKV", L".FLV", L".WMV", L".3GP", L".VOB", L".SWF", L".3G2", L".ASF", L".M2P", L".M2TS", L".MTS", L".M2V", L".M4V", L".MP2", L".OGM", L".OGV", L".QT", L".RM", L".RMVB", L".TS", L".WEBM" };
	...
	for (int k=0;k<uNumFiles;k++)
	{
		...
		{						
			if (!dirh.DirectoryExists((LPWSTR)m_szFile3))
			{
				...
				//MessageBox ( NULL ,szExt,szExt ,MB_ICONINFORMATION );

				szExtL=wcslwr(szExt);				
					 			
				if ((wcsstr(szFpL,szExtL))!=NULL)
				{
					isfiletype=true;
				}
				/*
...
HRESULT CExifRemoverShellExt::QueryContextMenu (
  ...
  if ( uFlags & CMF_DEFAULTONLY )
	...
	UINT upos=uMenuIndex;
		...
		if (lRet!=ERROR_SUCCESS)
		{
			...
			{
				InsertMenu ( hmenu, upos++, MF_BYPOSITION, uID++, L"Remove Exif Information");				
			}		

	if ( NULL != m_hApplicationBmp) 
    SetMenuItemBitmaps ( hmenu, upos-1, MF_BYPOSITION, m_hApplicationBmp, m_hApplicationBmp );		
...
HRESULT CExifRemoverShellExt::GetCommandString (
...
//	return E_FAIL;
  ...
  if (uFlags==GCS_VALIDATE)
  {
    ...
    {
    LPCTSTR szText = _T("Open in Exif Remover");	
...
HRESULT CExifRemoverShellExt::InvokeCommand(
...
{	
	uRetVal = GetTempFileName(lpPathBuffer, // directory for tmp files
                              TEXT("ERT"),     // temp file name prefix 
	...
	//lstrcatW(lpPathBuffer,_T("Exif Remover.txt"));
...
//lstrcatW(sParam,cmd);
	...
	//MessageBox ( NULL , sParam, _T("SimpleShlExt"),MB_ICONINFORMATION );	    
		...
		wcscat_s(szFp,_T("\\ExifRemover.exe") );
		...
		lstrcatW(szTempParam,_T(" -TempFile:\""));
